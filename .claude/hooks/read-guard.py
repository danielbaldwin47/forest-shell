...
PreToolUse on Read, blocking (exit 2) on either rule:
  ...
  2. Big first read: a whole-file Read of a guarded-extension repo file
     (GUARDED_EXT below) over BIG_FILE_LINES — the "ranged (grep first) on big
     files" rule. Here the escape is `limit`, since a bare `offset` still
     pulls Read's 2000-line default. A deliberate `offset: 1, limit: <n>` passes,
     so this is a speed bump rather than a wall.

Measured motivation (prior repo, 2026-08-05 transcript audit): read-after-edit
ran ~46% of sessions pre-rules and ~39% after prose alone — the one axis prose
never moved; and one session full-read a 656-line module three times under the
grep-first prose rule. See the claude-principles template repo,
principles/incidents.md; replace these numbers with this repo's own at the
first local audit.

EDIT AFTER COPYING: nothing is repo-specific; GUARDED_EXT and BIG_FILE_LINES
are deliberate defaults — change them only on local evidence.

starter-version: 2026-08-10 (claude-principles; locally tuned: GUARDED_EXT)
"""
import json
import os
import sys
...
GUARDED_EXT = {
    ...
    ".txt",
    ".log",
    # QML surfaces and policy objects are code like any other.
    ".qml",
}
